...
def ferryLoading(A,D):
    ...
    dp = [[False] * (D+1) for i in range(n+1)]
    parent = [[None] * (D+1) for i in range(n+1)]
    ...
    for k in range(n):
      for s in range(D+1):
        if dp[k][s] is True:
          ## jeżeli moge wpuscic tu, to wpuszczam kolejnyy
